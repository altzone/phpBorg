...
update_nginx_ssl() {
    ...
    if [ -S "/run/php/phpborg-8.3-fpm.sock" ]; then
        php_socket="/run/php/phpborg-8.3-fpm.sock"
...
server {
    ...
    location ~ ^/api/.*\.php\$ {
        root $PHPBORG_ROOT/api/public;
        fastcgi_pass unix:$php_socket;
        fastcgi_index index.php;
        fastcgi_param SCRIPT_FILENAME $PHPBORG_ROOT/api/public\$fastcgi_script_name;
    ...
    # Downloads endpoint (agent binary, etc.)
    location ~ ^/downloads/ {
        fastcgi_pass unix:$php_socket;
        fastcgi_index index.php;
        fastcgi_param SCRIPT_FILENAME $PHPBORG_ROOT/api/public/index.php;
